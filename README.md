...
왼쪽에 20×4칸짜리 커버(40×8픽셀)가 그려지고, 커버 높이만큼 남는 줄에 아티스트·앨범과 볼륨이 들어갑니다. 아트가 없는 곡은 같은 크기의 빈 판이 대신 들어가므로, 곡이 바뀌어도 밴드 높이와 텍스트 폭이 그대로입니다. 크기는 `hooks/register.tsx`의 `ART_COLUMNS`로 조절합니다.
