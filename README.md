...
| 모드 | 위치 | 커버 | 내용 |
| --- | --- | --- | --- |
| `compact` | 프롬프트 **아래** 힌트 줄 | 없음 | 제목 · 아티스트, 진행 미터, 전송 버튼 |
| `normal` (기본) | 프롬프트 **위** 밴드 (4줄) | 12×4 | 커버, 아티스트—앨범 줄, 셔플·볼륨 줄 |

`compact`는 재생 중이면 항상 떠 있습니다. 껐다 켤 것이 없으므로 `/player` 토글과 무관하며, 평소 `? for shortcuts`가 있던 자리를 대신 쓰고 Music.app이 멈추면 그 줄을 engine에 돌려줍니다. 상태줄과 겹치지 않게, compact가 트랙을 보여주는 동안에는 상태줄 표시를 내보내지 않습니다.
...
`compact` — 프롬프트 아래 힌트 줄에 한 줄. 아티스트가 제목 줄로 올라오고, 볼륨·셔플·반복 버튼은 빠집니다(명령으로 조작):
...
닫기 버튼도 없습니다 — 힌트 줄은 engine의 것이라, 재생이 멈추면 compact가 알아서 돌려줍니다.
