...
프롬프트 위에서 Apple Music을 조작하는 밴드. Apple 공식 인터페이스만 사용합니다.
...
| 모드 | 위치 | 커버 | 내용 |
| --- | --- | --- | --- |
| `compact` (기본) | 프롬프트 위 한 줄 | 없음 | 제목 · 아티스트, 진행 미터, 전송·탐색·크기 버튼 |
...
맨 끝은 크기 전환 버튼입니다. compact에서는 `▲`로 큰 밴드를 열고, normal에서는 `▼`로 한 줄로 접습니다 — 프롬프트에서 `/player`를 치는 것과 같은 동작이라, 밴드 안에서도 명령 없이 오갈 수 있습니다.

닫기 버튼은 없습니다. compact가 닫기의 종착지라서 더 숨길 곳이 없고, 재생이 멈추면 알아서 자리를 비웁니다.

`◀`와 `▶`는 10초씩 건너뜁니다. 진행바를 직접 클릭해 그 지점으로 점프하게 만들 수는 없습니다 — `ui.press`가 싣는 건 `{ plugin, element, component, surface }`뿐이고 포인터가 바의 몇 번째 칸에 떨어졌는지는 오지 않기 때문입니다. 바를 칸마다 Button으로 쪼개면 눌린 `key`가 위치를 대신하지만, Button에는 `[ ]` 크롬이 붙어서 `[█][░][░]`가 되어 더는 진행바로 읽히지 않습니다.

`seek`는 절대 위치를 받으므로 건너뛰기는 현재 위치에서 계산하고, 양 끝에서 잘라냅니다. 끝을 넘겨 탐색하면 다음 곡으로 넘어가 버리는데 그건 `▶▶`가 할 일입니다.

커버를 안 그리는 모드에서는 앨범아트를 아예 내보내지 않으므로, `compact`는 1초 폴링에 이미지 작업이 붙지 않습니다.

버튼 글리프는 **코딩 폰트에 실제로 들어 있는 것만** 씁니다. 처음 쓰던 `⏮ ⏪ ⏸ ⏩ ⏭ ⤨ 🔁 ✕`는 10개 중 8개가 PlemolJP Console NF에 없어서 글자마다 제각기 대체(fallback)됐습니다 — 일부는 텍스트 폰트로, 일부는 컬러 이모지 폰트로 떨어져 단색 글리프 사이에 파란 배지가 섞이고 폭까지 2칸으로 벌어졌습니다.

지금은 삼각형 한 계열로 통일했습니다. 건너뛰기는 삼각형 하나(`◀` `▶`), 곡 이동은 둘(`◀◀` `▶▶`)이라 같은 동작의 정도 차이로 읽힙니다. 반복은 `off → all → one` 3단계라 색만으로는 두 켜짐 상태를 구분할 수 없어 `one`일 때 `↻1`이 됩니다.

글리프를 바꿀 일이 생기면 폰트 `cmap`에 그 코드포인트가 있는지부터 확인하세요. 없으면 터미널이 알아서 다른 폰트에서 끌어오고, 그 폰트가 무엇일지는 통제할 수 없습니다.

버튼은 모두 같은 계열의 글리프라 한 줄의 컨트롤 스트립으로 읽힙니다. 켜져 있는 셔플·반복은 라벨이 길어지는 대신 색으로 표시되므로, 상태가 바뀌어도 줄이 밀리지 않습니다.
